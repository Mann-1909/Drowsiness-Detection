...
arduino_serial_conn = None # Global variable to talk to Arduino
...
# --- FUZZY SENSOR FUSION ENGINE ---

def fuzzy_ramp(val, safe_limit, danger_limit):
    """Linear piecewise function: returns 0.0 to 1.0 based on how far past safe_limit it is."""
    if val <= safe_limit: return 0.0
    if val >= danger_limit: return 1.0
    return (val - safe_limit) / (danger_limit - safe_limit)

def fuzzy_ramp_reverse(val, safe_limit, danger_limit):
    """Reverse ramp for things where LOWER is more dangerous."""
    if val >= safe_limit: return 0.0
    if val <= danger_limit: return 1.0
    return (safe_limit - val) / (safe_limit - danger_limit)

current_camera_score = 0.0

def calculate_total_risk(face_detected, eye_conf, yawn_conf, dist_conf):
    ...
    # Calculate total fuzzy penalty to add
    penalty = 0.0
    penalty += (SCORE_EYES_CLOSED * eye_conf)
    penalty += (SCORE_YAWN * yawn_conf)
    penalty += (SCORE_DISTRACTED * dist_conf)
    
    # Apply penalty or decay
    if penalty > 0:
        current_camera_score += penalty
    else:
        current_camera_score -= SCORE_DECAY
...
def run_arduino_bridge():
    ...
    for p in ports:
        try:
            ...
            arduino_serial_conn = arduino # Save connection for buzzer
            print(f"SUCCESS: Arduino Connected on {p}")
            
            print("--- Testing Arduino Buzzer (2 seconds) ---")
            try:
                arduino.write(b'1')
                time.sleep(2)
                arduino.write(b'0')
        ...
        try:
            if arduino.in_waiting > 0:
                ...
                if len(parts) >= 5:
                    ...
                    # --- FUZZY OVERRIDE FOR PHYSICAL SENSORS ---
                    alc_risk = fuzzy_ramp(alc_ppm, 20.0, ALCOHOL_LIMIT_PPM)
                    smoke_risk = fuzzy_ramp(smoke_ppm, 50.0, SMOKE_LIMIT_PPM)
                    
                    # BPM Fuzzy (Ignores < 10 for disconnected finger)
                    bpm_risk = 0.0
                    if bpm > 10:
                        bpm_high = fuzzy_ramp(bpm, 100.0, BPM_HIGH_THRESHOLD)
                        bpm_low = fuzzy_ramp_reverse(bpm, 60.0, BPM_LOW_THRESHOLD)
                        bpm_risk = max(bpm_high, bpm_low)
                        
                    # Temperature Fuzzy
                    temp1_risk = fuzzy_ramp(temp1, 35.0, TEMP_HIGH_THRESHOLD) # Cabin Temp
                    temp2_risk = fuzzy_ramp(temp2, 37.5, TEMP_HIGH_THRESHOLD) # Body Temp
                    
                    # Update global status based on highest risk
                    if alc_risk >= 1.0: 
                        sensor_data["status"] = "ALCOHOL DETECTED"
                    elif smoke_risk >= 1.0:
                        sensor_data["status"] = "SMOKE DETECTED"
                    elif bpm_risk >= 1.0 or temp1_risk >= 1.0 or temp2_risk >= 1.0:
                        sensor_data["status"] = "VITALS DANGER"
                    elif sensor_data["status"] not in ["BOOTING", "CALIBRATING"]:
...
def run_buzzer_control():
    ...
    while True:
        ...
        if status == "ALCOHOL DETECTED":
            ...
            control_buzzer(False)
            time.sleep(0.1)
        elif status == "VITALS DANGER":
            control_buzzer(True)
            if current_time - last_spoken_time > 5:
                speak_warning("Critical Vitals detected. Pull over.")
                last_spoken_time = current_time
            time.sleep(1.0)
            control_buzzer(False)
            time.sleep(0.2)
        ...
        try:
            ...
            if USE_MEDIAPIPE:
                ...
                if results.multi_face_landmarks:
                    ...
                    is_yawning = (mar > MAR_THRESHOLD) # Boolean for HUD drawing
                    
                    # FUZZY YAWN (Starts ramping at 0.3 MAR)
                    yawn_conf = fuzzy_ramp(mar, 0.3, MAR_THRESHOLD)
                    ...
                    avg_eye_score = (score_l + score_r) / 2.0
                    
                    # FUZZY EYES (Uses reverse ramp, safe is 0.7 open, danger is your threshold)
                    eye_conf = fuzzy_ramp_reverse(avg_eye_score, 0.7, EYE_CONFIDENCE_THRESHOLD)

                    if avg_eye_score <= EYE_CONFIDENCE_THRESHOLD: status_text = "Closed"
                        ...
                        if len(eyes) > 0:
                            for (ex, ey, ew, eh) in eyes:
                                ...
                                if score <= EYE_CONFIDENCE_THRESHOLD: 
                                    status_text = "Closed"
                                    eye_conf = 1.0 # Max penalty in fallback mode

            if face_detected:
                # FUZZY DISTRACTION (Taking maximum confidence among the 3 head axis)
                yaw_conf = fuzzy_ramp(abs(yaw), 15.0, YAW_DISTRACTION_THRESHOLD)
                pitch_conf = fuzzy_ramp(abs(pitch), 15.0, PITCH_THRESHOLD)
                roll_conf = fuzzy_ramp(abs(roll), 15.0, ROLL_THRESHOLD)
                
                dist_conf = max(yaw_conf, pitch_conf, roll_conf)
            
            # The calculation function now takes the fuzzy 0.0-1.0 multipliers
            final_risk = calculate_total_risk(face_detected, eye_conf, yawn_conf, dist_conf)
